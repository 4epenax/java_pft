...
public class SortingBubbles {
    ...
    public static void main(String[] args) {
        ...
        for (int j = 1; j < size; j++) {
            ...
            for (int i = 1; i < size; i++) {
                // если переменная массива слева больше, чем справа, то выполняется метод swap реализующий сдвиг
    ...
    public static void swap(int[] array, int n1, int n2) {
        ...
        if (array != null && array.length != 0) {
            ...
            // объявляется переменная, которая равняется первой(из двух) полученной из массива ~ буфер
            int temp = array[n1];
